...
from django_cron2.models import CronJobLog
...
class Schedule(object):
    def __init__(self, run_every_secs=None, run_at_times=[], retry_after_failure_mins=None):
...
class CronJobManager(object):
    ...
    def __should_run_now(self, cron_job, force=False):
        ...
        if force:
            return True
        if cron_job.schedule.run_every_secs != None:
            ...
            if previously_ran_successful_cron:
                if timezone.now() > previously_ran_successful_cron.start_time + timedelta(seconds=cron_job.schedule.run_every_secs):
                    return True
            else:
                return True
